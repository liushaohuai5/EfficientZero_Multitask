...
import numpy as np
from ez.envs import make_maniskill, make_envs
...
class EZManiSkillStateAgent(Agent):
    ...
    def update_config(self):
        ...
        env = make_maniskill(self.config.env.game, seed=self.config.env.base_seed, save_path=None, **self.config.env)
        obs = env.reset()
            ...
            with open_dict(self.config):
                if self.config.env.difficulty == 'medium':
                    self.config.env.env_names = [
                        'PushCube', 'PickCube', 'StackCube', 'PegInsertionSide', 'PushT', #'AnymalC-Reach',
                        # 'HumanoidPlaceAppleInBowl', 'OpenCabinetDrawer'
                    ]
                elif self.config.env.difficulty == 'hard':
                    raise NotImplementedError('Maniskill Large test set is not implemented yet, check https://maniskill.readthedocs.io/en/latest/user_guide/reinforcement_learning/baselines.html#standard-benchmark.')
