...
from ez.utils.format import get_ddp_model_weights, DiscreteSupport, symexp, log_std_transform, gaussian_logprob, squash, init_logger, grads_for_two_losses, cosine_from_grads, plot_tsne_with_labels
from ez.utils.loss import kl_loss, cosine_similarity_loss, symlog_loss, Value_loss
...
class Agent:
    ...
    def train(self, rank, replay_buffer, storage, batch_storage, logger):
        ...
        if os.path.exists(load_path):
            ...
            plot_tsne_with_labels(model.task_emb, self.config.env.env_names)
        ...
        while not (min(ray.get(replay_buffer.get_transition_num.remote())) >= self.config.train.start_transitions):
            time.sleep(1)
            pass
        ...
        while not self.is_finished(step_count):
            ...
            scalers, log_data = self.update_weights(model, batch, optimizer, replay_buffer, scaler, step_count, target_model=target_model, policy_optimizer=policy_optimizer)
            scaler = scalers[0]
            ...
            if is_main_process and step_count % pb_interval == 0:
                ...
                train_log_str = '[Train] {}, step {}/{}, {:.3f}h left. lr={:.3f}, avg time={:.3f}s, batchQ={}, '\
                                ...
                                ''.format(self.config.env.game, step_count, total_steps, left_time, lr, avg_time,
                                          batch_queue_size, self_play_reteurn,
                                          loss_data['loss/value_prefix'],
            ...
            if is_main_process and step_count % self.config.log.log_interval == 0:
                ...
                if len(eval_scalar) > 0:
                    ...
                    if not self.multi_task:
                        ...
                        eval_log_str = 'Eval {} at at step {}, score = {}' \
                                       ''.format(self.config.env.game, eval_counter, eval_scalar)
                        eval_logger.info(eval_log_str)
                        print('[Eval] ', eval_log_str)
                    logger.log({**{'eval_counter': eval_counter}, **eval_scalar})
                ...
                log_scalars.update({
                    'buffer/total_episode_num': sum(traj_num),
                    'buffer/total_transition_num': sum(transition_num)
                })
                if self.multi_task:
                    for task_idx, task_name in enumerate(self.config.env.env_names):
                        if not self.config.data.ind_exp_rp: task_idx = 0
                        log_scalars.update({
                            f'buffer/{task_name}_episode_num': traj_num[task_idx],
                            f'buffer/{task_name}_transition_num': transition_num[task_idx],
                        })

                log_distribution.update({
                    'dist/priorities_in_buffer': [prior for priorities in total_priorities for prior in priorities],
                })
                if self.multi_task:
                    for task_idx, task_name in enumerate(self.config.env.env_names):
                        if not self.config.data.ind_exp_rp: task_idx = 0
                        log_distribution.update({
                            f'buffer/{task_name}_priorities_in_buffer': total_priorities[task_idx],
                        })
    ...
    def continuous_loss(self, policy, target_action, target_policy, target_best_action, mask=None,
        ...
        if distribution_type == 'squashed_gaussian':
            ...
            # target_policy = target_policy.permute(1, 0)
            # loss = (-F.softmax(target_policy, dim=1) * policy_log_prob).sum(0)

            # Simple loss
            loss = -distr.log_prob(target_best_action)
        ...
        elif distribution_type == 'tdmpc2':
            mean, log_std = policy.chunk(2, dim=-1)
            log_std = log_std_transform(log_std, low=-10, dif=12)
            eps_target = (torch.atanh(target_best_action) - mean) / (log_std.exp() + 1e-6)
            log_prob = gaussian_logprob(eps_target, log_std)
            # # full loss
            # eps_targets = (torch.atanh(target_action) - mean.unsqueeze(1)) / (log_std.unsqueeze(1).exp() + 1e-6)
            # log_probs = gaussian_logprob(eps_targets, log_std.unsqueeze(1)) * target_policy.unsqueeze(-1)

            eps = torch.randn_like(mean)
            ent_log_prob = gaussian_logprob(eps, log_std)
            scaled_ent_log_prob = ent_log_prob * target_best_action.shape[-1]   # TODO: add multi-task

            _, _, squash_log_prob = squash(mean, torch.atanh(target_best_action), log_prob)
            # # full loss
            # _, _, squash_log_probs = squash(mean, torch.atanh(target_action), log_probs)
            # squash_log_prob = squash_log_probs.sum(dim=1)

            entropy_scale = scaled_ent_log_prob / (ent_log_prob + 1e-6)
            loss = -squash_log_prob.squeeze()

            entropy = (ent_log_prob * entropy_scale).squeeze()
    ...
    # @profile
    def update_weights(self, model, batch, optimizer, replay_buffer, scaler, step_count, target_model=None, policy_optimizer=None):
        target_model.eval()
        ...
        obs_batch_ori, action_batch, policy_prior_batch, mask_batch, indices, weights_lst, make_time, prior_lst, task_idx_lst = inputs_batch
        target_value_prefixes, target_values, target_actions, target_policies, target_best_actions, \
            top_value_masks, policy_masks, search_values = targets_batch
        ...
        task_idx_lst = torch.from_numpy(np.asarray(task_idx_lst).astype(np.int32)).long().cuda()

        if self.multi_task:
            expanded_masks = torch.from_numpy(np.asarray(self.config.env.action_masks)).float().cuda()[task_idx_lst]
        ...
        with autocast():
            states, values, policies = model.initial_inference(obs_batch, training=True, task_idxs=task_idx_lst[:, 0])
        value_lst.append(values)

        if self.config.model.value_support.type == 'symlog':
            scaled_value = symexp(values).mean(0)
        else:
            scaled_value = DiscreteSupport.vector_to_scalar(values, **self.config.model.value_support).mean(0)
        if self.continuous_action:
        ...
        if self.config.train.value_target == 'sarsa':
            ...
            if step_count < self.config.train.start_use_mix_training_steps:
                ...
                this_target_values = target_values * top_value_masks + search_values * (1 - top_value_masks)
        ...
        replay_buffer.update_priorities.remote(indices, fresh_priority, make_time, task_idx_lst[:, 0].detach().cpu().numpy())

        value_loss = torch.zeros(batch_size).cuda()
        value_loss += Value_loss(values, this_target_values[:, 0], self.config)
        # self.scale.update(DiscreteSupport.vector_to_scalar(values, **self.config.model.value_support).squeeze()[0])
        if self.continuous_action:
            mu, log_std = policies.chunk(2, dim=-1)
            ...
            other_distribution.update({
                'dist/policy_mu': mu.detach().cpu().numpy().flatten(),
                'dist/policy_sigma': sigma.detach().cpu().numpy().flatten(),
            })
            policy_loss, entropy_loss = self.continuous_loss(
            ...
            if self.multi_task:
                policies = policies.masked_fill(~expanded_masks[:, 0].bool(), -1e6)
                target_policies_0 = target_policies[:, 0] * expanded_masks[:, 0]    # pick-out active action dims
                target_policies_0 = target_policies_0 / (target_policies_0.sum(-1, keepdim=True) + 1e-6)  # renormalization
            else:
                target_policies_0 = target_policies[:, 0]
            policy_loss = kl_loss(policies, target_policies_0)
        ...
        path_consistency_loss = torch.zeros(batch_size).cuda()
        # unroll k steps recurrently
        with autocast():
            for step_i in range(unroll_steps):
                mask = mask_batch[:, step_i]
                states, value_prefixes, values, policies, reward_hidden = model.recurrent_inference(states, action_batch[:, step_i], reward_hidden, training=True, task_idxs=task_idx_lst[:, step_i+1])
                value_lst.append(values)
                reward_lst.append(value_prefixes)

                beg_index = image_channel * step_i
                end_index = image_channel * (step_i + n_stack)

                # consistency loss
                gt_next_states = model.do_representation(obs_target_batch[:, beg_index:end_index], task_idxs=task_idx_lst[:, step_i+1])
                ...
                if self.continuous_action:
                    policy_loss_i, entropy_loss_i = self.continuous_loss(
                    ...
                    if self.multi_task:
                        policies = policies.masked_fill(~expanded_masks[:, step_i+1].bool(), -1e6)
                        target_policies_i = target_policies[:, step_i + 1] * expanded_masks[:, step_i + 1]
                        target_policies_i = target_policies_i / (target_policies_i.sum(-1, keepdim=True) + 1e-6)
                    else:
                        target_policies_i = target_policies[:, step_i + 1]
                    policy_loss_i = kl_loss(policies, target_policies_i) * mask
                    policy_loss += policy_loss_i

                '''
                path consistency loss, proposed in 'Self-Consistent Models and Values' in NIPS2021
                '''
                if self.config.train.path_consistency:
                    this_value_prefixes = DiscreteSupport.vector_to_scalar(reward_lst[step_i], **self.config.model.reward_support).squeeze(-1)
                    prev_value_prefixes = DiscreteSupport.vector_to_scalar(reward_lst[step_i - 1], **self.config.model.reward_support).squeeze(-1) if step_i > 0 else torch.zeros_like(value_loss).squeeze(-1)
                    if self.config.model.value_prefix and (step_i % self.config.model.lstm_horizon_len != 0):
                        pred_rewards = this_value_prefixes - prev_value_prefixes
                    else:
                        pred_rewards = this_value_prefixes
                    this_values = DiscreteSupport.vector_to_scalar(value_lst[step_i + 1], **self.config.model.value_support).mean(0).squeeze(-1)      # min(0) is because of multi-value heads.
                    SC_targets = (pred_rewards + self.config.rl.discount * this_values).detach()
                    path_consistency_loss_i = Value_loss(value_lst[step_i], SC_targets, self.config) * mask   #* top_value_masks
                    path_consistency_loss += path_consistency_loss_i
        ...
        loss += policy_loss * self.config.train.policy_loss_coeff
        if self.continuous_action:
            loss += policy_entropy_loss * self.config.train.entropy_coeff

        loss += path_consistency_loss * self.config.train.value_loss_coeff
        weighted_loss = weights * loss

        if self.config.train.calc_grad_sim and (step_count + 1) % 100 == 0 and self.config.env.env == 'DMC':
            walk_idxs = torch.where(task_idx_lst[:, 0] == 1)[0]
            run_idxs = torch.where(task_idx_lst[:, 0] == 2)[0]
            crawl_idxs = torch.where(task_idx_lst[:, 0] == 4)[0]
            walk_losses = weighted_loss[walk_idxs].mean()
            run_losses = weighted_loss[run_idxs].mean()
            crawl_losses = weighted_loss[crawl_idxs].mean()
            params, gi, gj = grads_for_two_losses(walk_losses, run_losses, model)
            walk_run_grad_sim = cosine_from_grads(params, gi, gj, module=None)
            walk_run_grad_sim_dyn = cosine_from_grads(params, gi, gj, module=model.dynamics_model)
            walk_run_grad_sim_vp = cosine_from_grads(params, gi, gj, module=model.value_policy_model)
            walk_run_grad_sim_rew = cosine_from_grads(params, gi, gj, module=model.reward_prediction_model)

            # 再算一次（对 walk vs crawl）
            params, gi, gj = grads_for_two_losses(walk_losses, crawl_losses, model)
            walk_crawl_grad_sim = cosine_from_grads(params, gi, gj, module=None)
            walk_crawl_grad_sim_dyn = cosine_from_grads(params, gi, gj, module=model.dynamics_model)
            walk_crawl_grad_sim_vp = cosine_from_grads(params, gi, gj, module=model.value_policy_model)
            walk_crawl_grad_sim_rew = cosine_from_grads(params, gi, gj, module=model.reward_prediction_model)

        weighted_loss = weighted_loss.mean()

        if not weighted_loss.isfinite():
            import ipdb
            ipdb.set_trace()
            print('loss nan')
        ...
        loss_data.update({
            ...
            'loss/path_consistency': path_consistency_loss.mean().item(),
        })
        ...
        other_scalar.update({
            ...
            'other_log/scale_value': self.scale.value,
        })

        if self.config.train.calc_grad_sim and (step_count + 1) % 100 == 0 and self.config.env.env == 'DMC':
            other_scalar.update({
                'other_log/walk_run_grad_sim': walk_run_grad_sim.detach().cpu().numpy(),
                'other_log/walk_crawl_grad_sim': walk_crawl_grad_sim.detach().cpu().numpy(),
                'other_log/walk_run_grad_sim_dyn': walk_run_grad_sim_dyn.detach().cpu().numpy(),
                'other_log/walk_crawl_grad_sim_dyn': walk_crawl_grad_sim_dyn.detach().cpu().numpy(),
                'other_log/walk_run_grad_sim_vp': walk_run_grad_sim_vp.detach().cpu().numpy(),
                'other_log/walk_crawl_grad_sim_vp': walk_crawl_grad_sim_vp.detach().cpu().numpy(),
                'other_log/walk_run_grad_sim_rew': walk_run_grad_sim_rew.detach().cpu().numpy(),
                'other_log/walk_crawl_grad_sim_rew': walk_crawl_grad_sim_rew.detach().cpu().numpy(),
            })
        ...
        other_distribution.update({
            ...
            'dist/sampled_task_idx': task_idx_lst[:, 0].detach().cpu().numpy().flatten(),
        })
    ...
    def get_temperature(self, trained_steps):
        if self.config.train.change_temperature:
            ...
            if trained_steps < 0.5 * total_steps:   # prev 0.5
                ...
                return 0.25
            # else:
            #     return max(1 - max(trained_steps - 2e4, 0) / total_steps, 0.4)
        else:
            return 1.0

    def init_env(self, env, max_steps, seed=None):
        assert self._update
        if self.env_name == 'HumanoidBench':
        ...
        if self.multi_task:
            if not self.config.env.image_based:
                obs = np.concatenate([obs, np.zeros(self.config.env.obs_shape - obs.shape[0])])  # pad to same dim with 0
    ...
    def init_envs(self, envs, max_steps=None, seed=None):
        assert self._update

        stacked_obs_lst, game_trajs = [], []
        # initialization for envs, stack [n - 1 zero obs, current obs] for the n-stack obs
        for env in envs:
            stacked_obs, traj = self.init_env(env, max_steps, seed=seed)
...
class RunningScale:
    ...
    def update(self, x):
        ...
        self._value.data.lerp_(value, 0.01)
...
def train_ddp(agent, rank, replay_buffer, storage, batch_storage, logger):
    ...
    if rank == 0 and logger is None:
        if agent.config.env.multi_task:
            wandb_name = (f'{agent.config.env.env}-seed={agent.config.env.base_seed}-MT={agent.config.env.multi_task}-difficulty={agent.config.env.difficulty}'
                          f'-mppi={agent.config.mcts.use_mppi}')
        else:
            wandb_name = f'{agent.config.env.game}-seed={agent.config.env.base_seed}-mppi={agent.config.mcts.use_mppi}'
        ...
        logger.define_metric("eval_counter")
        logger.define_metric("eval/*", step_metric="eval_counter")
    else:
        logger = None
    # file logger
    log_path = os.path.join(agent.config.save_path, 'logs')
    ...
    is_main_process = (rank == 0)
    print(f'rank {rank} is main_process={is_main_process}')
    if is_main_process:
        train_logger = logging.getLogger('Train')
        eval_logger = logging.getLogger('Eval')
    ...
    if is_main_process: print(model)
    ...
    if os.path.exists(load_path):
        ...
        plot_tsne_with_labels(model.task_emb, agent.config.env.env_names)
    ...
    # wait until collecting enough data to start
    while not (min(ray.get(replay_buffer.get_transition_num.remote())) >= agent.config.train.start_transitions):
        time.sleep(1)
        pass
    print('[Train] Begin training...')
    ...
    while not agent.is_finished(step_count):
        ...
        #     lp = LineProfiler()
        #     lp_wrapper = lp(agent.update_weights)
        #     scaler, log_data = lp_wrapper(model, batch, optimizer, replay_buffer, scaler, step_count,
        #                                   target_model=target_model,
        #                                   )
        #     lp.print_stats()
        # else:
        scalers, log_data = agent.update_weights(model.module if agent.use_ddp else model, batch, optimizer, replay_buffer, scaler, step_count, target_model=target_model)
        scaler = scalers[0]
        ...
        if is_main_process and step_count % pb_interval == 0:
            ...
            batch_queue_size = batch_storage.get_len()
            train_log_str = '[Train] {}, step {}/{}, {:.3f}h left. lr={:.3f}, avg time={:.3f}s, batchQ={}, '\
                            'agent-play return={:.3f}, '\
                            'Loss: reward={:.3f}, value={:.3f}, policy={:.3f}, ' \
                            'consistency={:.3f}, entropy={:.3f}'\
                            ''.format(agent.config.env.game, step_count, total_steps, left_time, lr, avg_time,
                                      batch_queue_size, self_play_reteurn,
                                      # traj_num, transition_num / 1000, eval_score, eval_best_score,
                                      loss_data['loss/value_prefix'],
                                      loss_data['loss/value'], loss_data['loss/policy'],
                                      loss_data['loss/consistency'], loss_data['loss/entropy'])
            # print(f'target policy={batch[-1][-1][0, 0]}')
        ...
        if is_main_process and step_count % agent.config.log.log_interval == 0:
            ...
            if len(eval_scalar) > 0:
                eval_counter = ray.get(storage.get_eval_counter.remote())
                if not agent.config.env.multi_task:
                    eval_score = eval_scalar['eval/mean_score']
                    min_score, max_score = eval_scalar['eval/min_score'], eval_scalar['eval/max_score']
                    eval_best_score = ray.get(storage.get_best_score.remote())
                    ...
                    eval_log_str = 'Eval {} at at step {}, score = {}' \
                                   ''.format(agent.config.env.game, eval_counter, eval_scalar)
                    eval_logger.info(eval_log_str)
                    print('[Eval] ', eval_log_str)
                logger.log({**{"eval_counter": eval_counter}, **eval_scalar})
            ...
            if agent.config.env.multi_task:
                for task_idx, task_name in enumerate(agent.config.env.env_names):
                    if not agent.config.data.ind_exp_rp: task_idx = 0
                    log_scalars.update({
                        f'buffer/{task_name}_total_episode_num': traj_num[task_idx],
                        f'buffer/{task_name}_total_transition_num': transition_num[task_idx],
                    })

            log_distribution.update({
                f'dist/priorities_in_buffer': [prior for priorities in total_priorities for prior in priorities],
            })
            if agent.config.env.multi_task:
                for task_idx, task_name in enumerate(agent.config.env.env_names):
                    if not agent.config.data.ind_exp_rp: task_idx = 0
        ...
        log_scalars.update(loss_data)
        log_scalars.update(other_scalar)
        if is_main_process and step_count > 100 and step_count % agent.config.log.log_interval == 0:
            logger.log(log_scalars, step_count)
