...
import numpy as np
from ez.envs import make_humanoid_bench, make_envs
...
class EZHumanoidBenchStateAgent(Agent):
    ...
    def update_config(self):
        ...
        env = make_humanoid_bench(self.config.env.game, seed=self.config.env.base_seed, save_path=None, **self.config.env)
        obs = env.reset()
            ...
            with open_dict(self.config):
                if self.config.env.difficulty == 'medium':
                    self.config.env.env_names = [
                        'h1-stand', 'h1-walk', 'h1-run', 'h1-stair',
                        'h1-crawl', 'h1-pole', 'h1-slide', 'h1-hurdle', 'h1-maze'
                        # 'h1-stand', 'h1-stand', 'h1-stand', 'h1-stand', 'h1-stand',
                        # 'h1-stand', 'h1-stand', 'h1-stand', 'h1-stand'
                    ]
                elif self.config.env.difficulty == 'hard':
                    self.config.env.env_names = [
                        'h1hand-stand', 'h1hand-walk', 'h1hand-run', 'h1hand-stair', 'h1hand-crawl', 'h1hand-pole', 'h1hand-slide', 'h1hand-hurdle', 'h1hand-maze',
                        'h1hand-sit_simple', 'h1hand-sit_hard', 'h1hand-balance_simple', 'h1hand-balance_hard', 'h1hand-reach',
                        # 'h1hand-spoon', 'h1hand-window',
                        # 'h1hand-insert_small', 'h1hand-insert_normal',
                        # 'h1hand-bookshelf_simple', 'h1hand-bookshelf_hard'
                    ]
        ...
        task = self.config.env.game.split('-')[1]
        if task in 'reach':
            self.config.model.reward_support.range = [-20, 20]
            self.config.model.value_support.range = [-500, 500]
        ...
        # test discrete support
        x_test = torch.empty(100).uniform_(self.config.model.reward_support.range[0], self.config.model.reward_support.range[1])
        x_support = reward_support.scalar_to_vector(x_test, **self.config.model.reward_support)
        x_logits = torch.log(x_support).clip(-1e6, 1000)
        x_transformed = reward_support.vector_to_scalar(x_logits, **self.config.model.reward_support)
        assert (x_transformed.squeeze() - x_test).abs().mean() < 1e-2

        x_test = torch.empty(100).uniform_(self.config.model.value_support.range[0], self.config.model.value_support.range[1])
        x_support = value_support.scalar_to_vector(x_test, **self.config.model.value_support)
        x_logits = torch.log(x_support).clip(-1e6, 1000)
        x_transformed = value_support.vector_to_scalar(x_logits, **self.config.model.value_support)
        assert (x_transformed.squeeze() - x_test).abs().mean() < 1e-2
        ...
        with open_dict(self.config):
            ...
            # self.config.mcts.c_scale = self.config.mcts.num_simulations * 0.1
            ...
            if self.multi_task:
                ...
                # self.config.train.training_steps *= 4
    ...
    def build_model(self):
        ...
        representation_model = RepresentationNetwork(self.obs_shape, self.n_stack, self.num_blocks,
                                                     ...
                                                     use_bn=self.use_bn, multi_task=task_embedding_choice[0])
        ...
        dynamics_model = DynamicsNetwork(self.hidden_shape, self.action_space_size, self.num_blocks, self.dyn_shape,
                                         ...
                                         use_bn=self.use_bn, multi_task=task_embedding_choice[1])
        ...
        value_policy_model = VP_model(
            ...
            v_num=self.v_num, multi_task=task_embedding_choice[3]
        )
        ...
        if self.config.model.reward_parallel:
            reward_prediction_model = RewardNetworkParallel(
                ...
                action_embedding=self.config.model.action_embedding, multi_task=task_embedding_choice[2]
            )
        else:
            reward_prediction_model = RewardNetwork(
                self.hidden_shape, self.task_embedding_shape,
                self.rew_net_shape, reward_output_size, self.num_blocks, init_zero=self.init_zero, use_bn=self.use_bn,
                multi_task=task_embedding_choice[2]
            )
