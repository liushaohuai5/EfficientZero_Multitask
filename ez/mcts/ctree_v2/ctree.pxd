...
cdef extern from "gumbel_cnode.h" namespace "tree":
    cdef cppclass CNode:
        ...
        void expand(int to_play, int hidden_state_index_x, int hidden_state_index_y, float reward_sums, vector[float] policy_logits, int simulation_num, int leaf_num)
    ...
    cdef cppclass CRoots:
        ...
        void prepare(const vector[float] &reward_sums, const vector[vector[float]] &policies, int m, int simulation_num, const vector[float] &values, int leaf_num)
    ...
    cdef cppclass CSearchResults:
        ...
        CSearchResults(int num) except +
        int num
        vector[int] hidden_state_index_x_lst, hidden_state_index_y_lst, last_actions, first_actions, search_lens
        vector[CNode*] nodes
        # vector[vector[CNode*]] search_paths
    ...
    cdef void cback_propagate(vector[CNode*] &search_path, CMinMaxStats &min_max_stats, int to_play, float value, float discount)
    void cmulti_back_propagate(int hidden_state_index_x, float discount, vector[float] reward_sums, vector[float] values, vector[vector[float]] policies,
                               CMinMaxStatsList *min_max_stats_lst, CSearchResults &results, vector[int] is_reset_lst, int simulation_idx, vector[vector[float]] gumbels, float c_visit, float c_scale, int simulation_num, int leaf_num)
