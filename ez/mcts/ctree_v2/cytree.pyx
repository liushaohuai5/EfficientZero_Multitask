...
cdef class Node:
    ...
    def expand(self, int to_play, int hidden_state_index_x, int hidden_state_index_y, float reward_sum, list policy_logits, int simulation_num, int leaf_num):
        cdef vector[float] cpolicy = policy_logits
        self.cnode.expand(to_play, hidden_state_index_x, hidden_state_index_y, reward_sum, cpolicy, simulation_num, leaf_num)
...
cdef class Roots:
    ...
    def prepare(self, list reward_sum_pool, list policy_logits_pool, int m, int simulation_num, list values, int leaf_num):
        self.roots[0].prepare(reward_sum_pool, policy_logits_pool, m, simulation_num, values, leaf_num)
...
def multi_back_propagate(int hidden_state_index_x, float discount, list reward_sums, list values, list policies, MinMaxStatsList min_max_stats_lst, ResultsWrapper results, list is_reset_lst, int simulation_idx, list gumbels, float c_visit, float c_scale, int simulation_num, int leaf_num):
    cdef int i
    cdef vector[float] creward_sums = reward_sums
    cdef vector[float] cvalues = values
    cdef vector[vector[float]] cpolicies = policies
    ...
    cmulti_back_propagate(hidden_state_index_x, discount, creward_sums, cvalues, cpolicies,
                          min_max_stats_lst.cmin_max_stats_lst, results.cresults, cis_reset_lst, simulation_idx, cgumbels, c_visit, c_scale, simulation_num, leaf_num)
...
def multi_traverse(Roots roots, float c_visit, float c_scale, float discount, MinMaxStatsList min_max_stats_lst, ResultsWrapper results, int simulation_idx, list gumbels, int use_transformer):
    ...
    if use_transformer == 0:
        ...
        return results.cresults.hidden_state_index_x_lst, results.cresults.hidden_state_index_y_lst, results.cresults.last_actions, results.cresults.first_actions, results.cresults.search_lens, None
    else:
        # print('here1')
