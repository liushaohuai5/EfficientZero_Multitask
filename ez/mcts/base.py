...
class MCTS:
    ...
    def sample_mpc_actions(self, policy):
        is_continuous = (self.env in ['DMC', 'Gym', 'Humanoid_Bench', 'ManiSkill'])
        if is_continuous:
            mean, _ = policy.chunk(2, dim=-1)
            return mean
        else:
    ...
    def update_statistics(self, **kwargs):
        if kwargs.get('prediction'):
            ...
            for _ in range(self.mpc_horizon):
                with torch.no_grad():
                    with autocast():
                        states, pred_value_prefixes, next_values, next_logits, reward_hidden, variances = \
            ...
            self.log('simulate action {}, r = {:.3f}, v = {:.3f}, logits = {}'
                     ...
                     verbose=3)
            return states, next_value_prefixes, next_values, next_logits, reward_hidden, variances
        else:
    ...
    def estimate_value(self, **kwargs):
        ...
        for i in range(actions.shape[0]):
            ...
            with torch.no_grad():
                with autocast():
                    next_states, next_value_prefixes, next_values, next_logits, reward_hidden, variances = model.recurrent_inference(current_states, actions[i], reward_hidden)
