...
class GameTrajectory:
    def __init__(self, **kwargs):
        ...
        self.auto_td_steps = kwargs.get('auto_td_steps')
    ...
    def append(self, action, obs, reward, policy_prior=None, task_idx=None):
        ...
        if policy_prior is not None:
            self.policy_prior_lst.append(policy_prior)
        if task_idx is not None:
            self.task_idx_lst.append(task_idx)

    def pad_over(self, tail_obs, tail_rewards, tail_pred_values, tail_search_values, tail_policies, tail_priors=None, tail_task_idxs=None):
        ...
        if tail_priors is not None:
            for policy_prior in tail_priors:
                self.policy_prior_lst.append(policy_prior)

        if tail_task_idxs is not None:
            for task_idx in tail_task_idxs:
                self.task_idx_lst.append(task_idx)
    ...
    def save_to_memory(self):
        ...
        # convert to numpy
        if self.image_based:
            self.obs_lst = ray.put(np.array(self.obs_lst).astype(np.float32))
        else:
            self.obs_lst = np.array(self.obs_lst).astype(np.float32)
        self.reward_lst = np.array(self.reward_lst).astype(np.float32)
        self.policy_lst = np.array(self.policy_lst).astype(np.float32)
        self.policy_prior_lst = np.array(self.policy_prior_lst).astype(np.float32)
        self.action_lst = np.array(self.action_lst).astype(np.float32)
        self.pred_value_lst = np.array(self.pred_value_lst).astype(np.float32)
        self.search_value_lst = np.array(self.search_value_lst).astype(np.float32)
        self.bootstrapped_value_lst = np.array(self.bootstrapped_value_lst).astype(np.float32)
        self.task_idx_lst = np.array(self.task_idx_lst).astype(np.int32)
    ...
    def make_target(self, index):
        ...
        target_policy_prior = self.policy_prior_lst[index:index+self.unroll_steps+1]

        assert len(target_reward) == len(target_pred_value) == len(target_search_value) == len(target_policy)
        return np.array(target_obs), np.array(target_reward), np.array(target_pred_value), np.array(target_search_value), np.array(target_bt_value), np.array(target_policy), np.array(target_policy_prior)
    ...
    def get_monte_carlo_return(self, prev_return=0, prev_steps=0, last_value=0):
        self.monte_carlo_return += prev_return
        for i in range(len(self.reward_lst)):
            self.monte_carlo_return += self.reward_lst[i] * (self.discount ** (i+prev_steps))
        self.monte_carlo_return += last_value * (self.discount ** (prev_steps+len(self.reward_lst)))
        return self.monte_carlo_return
    ...
    def get_gae_value(self, value_type='prediction', value_lst_input=None, index=None, collected_transitions=None):
        ...
        if index is None:
            ...
            delta_lambda = 0.1 * (
                    collected_transitions - index) / self.auto_td_steps
    ...
    def get_bootstrapped_value(self, value_type='prediction', value_lst_input=None, index=None, collected_transitions=None):
        ...
        if index is None:
            ...
            delta_td = (collected_transitions - index) // self.auto_td_steps
    ...
    def get_index_stacked_obs(self, index, padding=False, extra=0):
        ...
        if padding:
            ...
            if pad_len > 0:
                # if len(frames) > 0:
                #     assert np.all(frames[-1] == self.obs_lst[-1])
